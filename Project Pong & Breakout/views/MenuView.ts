class MenuView extends BaseView {
    ...
    public init(): Promise<void> {
        ...
        var interfaceVolume = 'setting:interfaceVolume' in localStorage ? Number(localStorage['setting:interfaceVolume']) : 0.5;
        ...
        return Promise.all([
            MenuViewModel.Create(app).then((viewModel) => {
            ...
            for (var i = 0; i < this.viewModel.Actions.length; i++) {
                ...
                action.subscribe("changed", (caller, data) => {
                    ...
                    else if (data.property == "Selected" && data.newValue && caller instanceof ActionItemViewModel) {
            ...
            var registerAction = (action: ActionItemViewModel) => {
                action.subscribe("changed", (caller, data) => {
                    if (data.property == "Focus" && data.newValue) {
